...
final class RentalCommercialPricingService {
 ...
 public function quote(ReservationSearch $search,array $itemIds=[],?string $couponCode=null):array{
  ...
  return ['rate_plan'=>['id'=>$rate->id,'code'=>$rate->code,'name'=>$rate->name],'base'=>$base->toArray(),'commercial_items'=>$items->values()->all(),'gross_total'=>$gross,'coupon_discount'=>$discount,'deposit_value'=>round((float)$rate->deposit_value+$items->sum('deposit_adjustment'),2),'total_value'=>round(max(0,$gross-$discount),2),'commercial_rules'=>RentalCommercialRule::query()
            ->withoutOrganizationScope()->where('organization_id',$search->organizationId)->where('status','active')->orderBy('priority')->get()->toArray()];
 }
 private function rate(ReservationSearch $s):?RentalRatePlan{return RentalRatePlan::query()
            ->withoutOrganizationScope()->where('organization_id',$s->organizationId)->where('status','active')->where(fn(Builder $q)=>$q->whereNull('branch_id')->orWhere('branch_id',$s->branchId))->where(fn(Builder $q)=>$q->whereNull('asset_category_id')->orWhere('asset_category_id',$s->categoryId))->orderBy('priority')->first();}
